...
async def cmd_tried(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Добавляем 4 часа к последнему кормлению (или к текущему времени если было давно)
    last_feed_str = data.get("last_feed_time")
    if last_feed_str:
        last_feed = datetime.fromisoformat(last_feed_str)
        # Если уже в режиме "плохо" — добавляем от текущего времени
        if get_hunger_mode(data) != "good":
            new_feed_time = now_msk() - timedelta(hours=8)  # 12-4=8, значит до голода 4ч
        else:
            new_feed_time = last_feed + timedelta(hours=4)
            # Но не больше чем "сейчас"
            if new_feed_time > now_msk():
                new_feed_time = now_msk() - timedelta(hours=8)
    else:
        new_feed_time = now_msk() - timedelta(hours=8)
